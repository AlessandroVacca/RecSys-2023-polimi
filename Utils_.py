...
class EarlyStoppingScheduler(object):
    ...
    def score(self, epoch):
        if epoch % self.freq == 0:
            ...
            curr_scores = np.array([results_dic[m] for m in self.metrics])
            print('Scores at epoch %d: %s' % (epoch, str(curr_scores)))
...
def plot_loss_acc(model, dict_values, xlabel='epochs', ylabel=None, scale='linear'):
    ...
    config_list = ['d_nodes', 'g_nodes', 'g1_nodes', 'g2_nodes', 'd_hidden_act', 'g_hidden_act', 'g_output_act',
                   'use_dropout', 'use_batchnorm', 'dropout', 'batch_mom', 'epochs', 'sgd_var', 'adam_var', 'sgd_mom',
                   'beta1']
...
def plot_gradients(gradients):
    ...
    for i, l in enumerate(layers):
        g = sns.FacetGrid(df.iloc[:, df.layer == l], row='epochs', hue='epochs', aspect=15, height=.5, palette=pal,
                          ax=ax[0, i])
